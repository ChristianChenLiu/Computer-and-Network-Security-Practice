...
def encrypt(mode, keyFile, inputFile, outputFile):
	...
	if mode == 'ecb':
		...
		raise Exception("Invalid encryption mode, please use one of ecb, cbc or ctr modes")

	#Encrypt the Image
	cipherImage = cipher.encrypt(content)

	outputImage = Image.frombytes('RGB', inputImage.size, cipherImage)
...
def decrypt(mode, keyFile, inputFile, outputFile):
	...
	if mode == 'ecb':
		...
		raise Exception("Invalid decryption mode, please use one of ecb, cbc or ctr modes")

	#Decrypt the Image
	originalImage = cipher.decrypt(content)S

	outputImage = Image.frombytes('RGB', inputImage.size, originalImage)
	outputImage.save(outputFile)
	inputImage.close()
	outputImage.close()
	file.close()
